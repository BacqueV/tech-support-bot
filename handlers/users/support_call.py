...
from keyboards.inline.support import support_keyboard, support_callback, \
    check_support_available, get_support_manager, cancel_support, cancel_support_callback


@dp.message_handler(Command('live_chat'))
async def call_an_operator(message: types.Message):
    text = 'Want to call an operator? You can ask them by clicking the button below!'
...
async def connect_with_operator(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    await call.message.edit_text('You are connected with operator. Please wait his answer!')
    ...
    if not support_id:
        await call.message.edit_text('There are no available operators.')
    ...
    await bot.send_message(
        support_id, f'{call.from_user.full_name} needs help in live chat!',
        reply_markup=keyboard
    )


@dp.callback_query_handler(support_callback.filter(messages='many', as_user='no'))
async def answer_call(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    if str(await user_state.get_state()) != 'wait_in_support':
        await call.message.edit_text("User doesn't need help no more")
        return
    ...
    keyboard_user2 = cancel_support(call.from_user.id)

    await call.message.edit_text(
        'You are having a live chat with user!\n'
        'You can cancel it clicking the button below.',
        reply_markup=keyboard
    )
    await bot.send_message(
        second_id,
        'Tech support is online, you can start chatting!\n'
        'You can cancel it clicking the button below.',
        reply_markup=keyboard_user2
    )
...
async def not_supported(message: types.Message, state: FSMContext):
    ...
    keyboard = cancel_support(second_id)
    await message.answer('Wait until agent answer you or cancel live chat.',
                         reply_markup=keyboard
                         )
...
async def exit_support(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    if await second_state.get_state() is not None:
        ...
        if int(second_id) == call.from_user.id:
            ...
            await bot.send_message(user_id, 'User canceled live chat session')

    await call.message.edit_text('You canceled live chat session')
    await state.reset_state()
